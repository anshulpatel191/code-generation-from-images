...
#     paths = []
#     for f in os.listdir(input_path):
#         if f.endswith(".txt"):  # Check for text files instead of GUI files
#             file_name = os.path.splitext(f)[0]  # Extract the file name without extension
#             if os.path.isfile(os.path.join(input_path, file_name + ".jpg")):
#                 paths.append(file_name)
...
#     hashes = []
#     for path in paths:
#         with open(os.path.join(input_path, path + ".txt"), 'r', encoding='utf-8') as f:  # Open text file for reading
#             chars = f.read()  # Read the entire content of the text file
#             content_hash = hashlib.sha256(chars.encode('utf-8')).hexdigest()  # Compute the hash of the content
...
#             is_unique = True
#             for h in hashes:
#                 if h == content_hash:  # Compare hashes for uniqueness
#                     is_unique = False
#                     break
...
#     if not os.path.exists(os.path.join(os.path.dirname(input_path), EVALUATION_SET_NAME)):
#         os.makedirs(os.path.join(os.path.dirname(input_path), EVALUATION_SET_NAME))

#     if not os.path.exists(os.path.join(os.path.dirname(input_path), TRAINING_SET_NAME)):
#         os.makedirs(os.path.join(os.path.dirname(input_path), TRAINING_SET_NAME))
    
#     for path in test_set:
#         shutil.copyfile(os.path.join(input_path, path + ".jpg"), os.path.join(os.path.dirname(input_path), TESTING_SET_NAME, path + ".jpg"))
#         shutil.copyfile(os.path.join(input_path, path + ".txt"), os.path.join(os.path.dirname(input_path), TESTING_SET_NAME, path + ".txt"))

#     for path in eval_set:
#         shutil.copyfile(os.path.join(input_path, path + ".jpg"), os.path.join(os.path.dirname(input_path), EVALUATION_SET_NAME, path + ".jpg"))
#         shutil.copyfile(os.path.join(input_path, path + ".txt"), os.path.join(os.path.dirname(input_path), EVALUATION_SET_NAME, path + ".txt"))

#     for path in train_set:
#         shutil.copyfile(os.path.join(input_path, path + ".jpg"), os.path.join(os.path.dirname(input_path), TRAINING_SET_NAME, path + ".jpg"))
...
def split_dataset(input_path):
    '''
    Split dataset into train, eval, and test sets
    :param input_path: path to location of all images and associated txt code
    :return: None
    '''
    TRAINING_SET_NAME = "img/train_images"
    EVALUATION_SET_NAME = "img/eval_images"
    ...
    for f in os.listdir(input_path):
        if f.endswith(".txt"):  # Check for text files instead of GUI files
            file_name = os.path.splitext(f)[0]  # Extract the file name without extension
            if os.path.isfile(os.path.join(input_path, file_name + ".jpeg")):
                paths.append(file_name)

    np.random.shuffle(paths)

    total_samples = len(paths)
    training_samples_number = int(0.7 * total_samples)
    evaluation_samples_number = int(0.2 * total_samples)
    testing_samples_number = total_samples - training_samples_number - evaluation_samples_number
